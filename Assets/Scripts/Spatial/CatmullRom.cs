...
static class CatmullRom
{
    ...
    public static void SmoothPath(IList<Vector3> base_path, IList<Vector3> smoothed_path, int subdivisions)
    {
        int total_points = base_path.Count;
        if (subdivisions <= 0 || total_points < 3)
        {
            //Nothing to smooth
        ...
        //Start segment
        SmoothSegment(start_cap, base_path[0], base_path[1], base_path[2], smoothed_path, subdivisions);

        //Main Body
        int last_control_point = total_points - 3;
        for (int i = 0; i < last_control_point; ++i)
        {
            SmoothSegment(base_path[i], base_path[i + 1], base_path[i + 2], base_path[i + 3], smoothed_path, subdivisions);
        }

        //End Segment
        SmoothSegment(base_path[total_points - 3], base_path[total_points - 2], base_path[total_points - 1], end_cap, smoothed_path, subdivisions);
    ...
    private static void SmoothSegment(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, IList<Vector3> smoothed_path, int subdivisions)
    {
        //Add the start
        ...
        float point_t = (t2 - t1) / subdivisions;
        for (int i = 1; i <= subdivisions; ++i) // 1 <= to ensure t is always at least point_t
        {
